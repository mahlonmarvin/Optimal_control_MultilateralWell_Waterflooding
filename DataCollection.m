...
ln=1; nT=10; pert=0;
OilTraj2=[]; OilTraj3=[]; OilTraj4=[];
WProTraj2=[]; WProTraj3=[]; WProTraj4=[];
WInjTraj1=[];
U1Traj=[];
NPVTraj=[];
...
while ln<=nT,
    ...
    if ln>3,        
pert1=(rand(1)/10000+0.0003)/day;
W(1).val=controls.well(1).values(1)+pert1; 
W(2).val=-W(1).val; W(3).val=-W(1).val; W(4).val=-W(1).val; 

    else
pert1=0;
W(1).val=controls.well(1).values(1);
W(2).val=-W(1).val; W(3).val=-W(1).val; W(4).val=-W(1).val;        
    end

rISol = initState(G, W, 0, [0, 1]);   
gravity off;
rISol = incompMimetic(rISol, G, S, fluid, 'wells', W);


%% Start the main loop
t  = 0; plotNo = 1; hi = 'Implicit: '; he = 'Explicit: ';
po2 = []; po3 = []; po4 = []; pw2 = []; pw3 = []; pw4 = [];
uw1 = []; NPV1=0; NPV=[]; l=1; 
uc1 = [];
...
while t < T,
   ...
   W(1).val=controls.well(1).values(l)+pert1; 
  W(2).val=-W(1).val; W(3).val=-W(1).val; W(4).val=-W(1).val;  
...
OilTraj2=[OilTraj2, po2];
OilTraj3=[OilTraj3, po3];
OilTraj4=[OilTraj4, po4];
WProTraj2=[WProTraj2, pw2];
WProTraj3=[WProTraj3, pw3];
WProTraj4=[WProTraj4, pw4]; 
WInjTraj1=[WInjTraj1, uw1];
